    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 20) {
                ...
                ZStack {
                    RoundedRectangle(cornerRadius: 18).fill(Pastel.tile)
                    RoundedRectangle(cornerRadius: 18).stroke(Pastel.tileStroke, lineWidth: 1)
                    VStack(spacing: 10) {
                        Text("Current Letter")
                            .font(.headline)
                ...
                NavigationLink("Play", destination: GameView().environmentObject(core))
                    .buttonStyle(.borderedProminent)

                Button("Reset progress", role: .destructive) { core.resetProgress() }
                ...
                ToolbarItem(placement: .topBarTrailing) {
                    Button {
                        if let top = UIApplication.shared.topMostViewController() {
                            GCManager.shared.presentLeaderboard(from: top, letterIndex: core.progress.currentIndex)
                        }
